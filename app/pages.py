...
from logic import generate_lesson, generate_style_sample, pdf_upload
...
def render_style_sample_page():
    ...
    with col_input:
        style_prompt = st.text_input(
            "Внешний вид", placeholder="Цвет, шрифт, размер, фон…"
        )
        structure_prompt = st.text_input(
            "Структура", placeholder="Порядок изложения, разбиение на части…"
        )
        ...
        if st.button("Создать шаблон"):
            ...
            if not structure_prompt:
                structure_prompt = (
                    "Введение, основная часть с bullet списком и заключение"
                )
            ...
            try:
                ...
                tmpl = create_template_with_s3(
                    db=db, title=title, author_id=author_id, html=sample_html
                )
            finally:
                db.close()
...
def render_lesson_page():
    ...
    with col_input:
        ...
        if not st.session_state.get("generated_lesson") and current_lesson.get(
            "content"
        ):
        ...
        lesson_prompt = st.text_area(
            "Запрос для урока",
            default,
            placeholder="Опишите ваш желаемый контент здесь",
        )
        ...
        with col_save:
            if st.button("Сохранить урок"):
                if not st.session_state.get("current_lesson"):
                    ...
                    try:
                        lesson_name = (
                            st.session_state.current_lesson.get("prompt")[:20]
                            or "Новый урок"
                        )
                        tpl = next(
                            t
                            for t in templates
                            if t.title
                            == st.session_state.current_lesson["selected_template"]
                        )
